...
function loadCityIndex(key) {
    ...
    function parseInboxReport() {
        ...
        if (reportElement != null) {
            ...
            if (footerText.indexOf('gd_index_rep_') < 0
                ...
                for (var j = 0; j < gd_w.reportsFoundInbox.length; j++) {
                    if (gd_w.reportsFoundInbox[j] === reportHash) {
                        reportFound = true;
                    }
                }
                if (reportFound) {
            ...
            for (var i = 0; i < reportsInView.length; i++) {
                ...
                if (!$('#gd_index_f_' + reportId).get(0)) {
                    ...
                    if (reportHash !== null && reportHash !== 0) {
                        for (var j = 0; j < gd_w.reportsFoundForum.length; j++) {
    ...
    function settings() {
        if (!$("#gd_indexer").get(0)) {
            ...
            gd_w.gdIndexScript.forEach(function (index, i) {
...
function enableCityIndex(key) {
    if (gd_w.gdIndexScript === undefined) {
        ...
        }, 4000 * (gd_w.gdIndexScript.length - 1));
    }
}
...
if(gd_w.location.href.indexOf("grepodata.com") >= 0){
    ...
    function grepodataObserver(path) {
        var initWatcher = setInterval(function () {
            if (gd_w.location.pathname.indexOf("/indexer/") >= 0 &&
                gd_w.location.pathname.indexOf("{/literal}{$key}{literal}") >= 0 &&
  ...
  // Indexer (in-game)
  enableCityIndex("{/literal}{$key}{literal}");
}
